...
public class MarketServer extends Thread {
    ...
    // Thư mục lưu file trên server (Đảm bảo bạn đã tạo thư mục này trên ổ đĩa
    // Server)
    ...
    private void handleClient(Socket socket) {
        try (DataInputStream dis = new DataInputStream(socket.getInputStream());
            ...
            if ("UPLOAD_SELL".equals(command)) {
                ...
                boolean success = DatabaseManager.insertMarketFile(fileName, fileSize, saveFile.getAbsolutePath(),
                        sellerId, price);
                ...
                String status = DatabaseManager.buyFile(buyerId, fileId);
                dos.writeUTF(status);
                dos.flush();
            ...
            } else if ("GET_USER_POINTS".equals(command)) {
                int userId = dis.readInt();
                long points = DatabaseManager.getUserPoints(userId);
                dos.writeLong(points);
                dos.flush();
                ...
                int sellerId = DatabaseManager.getFileSeller(fileId);
                if (userId == sellerId) {
                    dos.writeBoolean(false);
                    dos.writeUTF("Bạn không thể download file do chính mình đăng bán.");
                    dos.flush();
                    return;
                }

                long price = DatabaseManager.getFilePrice(fileId);
                boolean hasBought = DatabaseManager.hasUserBoughtFile(userId, fileId);
                if (price > 0 && !hasBought) {
                    dos.writeBoolean(false);
                    dos.writeUTF("Bạn chưa mua file này, không thể download!");
                    dos.flush();
                    return;
                }
